...
abstract class Translates extends Functions
{
    ...
    public function getTranslate(string $text, string $lang = ''): string {
        ...
        foreach ($this->_applicationModulesInstance->getModulesTranslateListName() as $module) {
            if (isset($module::TRANSLATE_TABLE[$lang][$text]))
                return $module::TRANSLATE_TABLE[$lang][$text];
        }
        foreach ($this->_applicationModulesInstance->getModulesListInstances() as $instance) {
            if (isset($instance::TRANSLATE_TABLE[$lang][$text]))
                return $instance::TRANSLATE_TABLE[$lang][$text];
        }

        if (isset(Translates::TRANSLATE_TABLE[self::DEFAULT_LANGUAGE][$text]))
        ...
        foreach ($this->_applicationModulesInstance->getModulesTranslateListName() as $module) {
            if (isset($module::TRANSLATE_TABLE[self::DEFAULT_LANGUAGE][$text]))
                return $module::TRANSLATE_TABLE[self::DEFAULT_LANGUAGE][$text];
        }
        foreach ($this->_applicationModulesInstance->getModulesListInstances() as $instance) {
            if (isset($instance::TRANSLATE_TABLE[self::DEFAULT_LANGUAGE][$text]))
