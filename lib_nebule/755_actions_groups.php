...
class ActionsGroups extends Actions implements ActionsInterface {
    ...
    const ADD_MEMBER = 'action_group_add_member';
    const ADD_MEMBER_OBFUSCATED = 'action_group_add_member_obf';
    ...
    const CREATE_MEMBER_GROUP_TYPED = 'action_group_create_member_type';
...
//        $this->_metrologyInstance->addLog('track functions', Metrology::LOG_LEVEL_FUNCTION, __METHOD__, '1111c0de');
        ...
        try {
            ...
            $instance = $this->_cacheInstance->newGroup($gid);
            if ($instance->getID() == '0')
                return;
...
//        $this->_metrologyInstance->addLog('track functions', Metrology::LOG_LEVEL_FUNCTION, __METHOD__, '1111c0de');
        ...
        try {
            ...
            $obfuscated = ($this->_configurationInstance->getOptionAsBoolean('permitObfuscatedLink') && $this->getHaveInput(self::ADD_MEMBER_OBFUSCATED));
            if ($typed == '')
...
//        $this->_metrologyInstance->addLog('track functions', Metrology::LOG_LEVEL_FUNCTION, __METHOD__, '1111c0de');
        ...
        try {
            ...
            $typed = $this->getFilterInput(self::CREATE_MEMBER_GROUP_TYPED, FILTER_FLAG_NO_ENCODE_QUOTES);
            ...
            if ($content != '') {
                if ($index == '') {
                    ...
                    $nodeContent->write();
                    if ($typeMime != '')
                        $nodeContent->setType($typeMime);
                    $nid = $instance->getID();
                    $this->_metrologyInstance->addLog('create group gid=' . $nid . ' with content nid=' . $nodeContent->getID() . ' and index=' . $index, Metrology::LOG_LEVEL_AUDIT, __METHOD__, '219474b1');
                    $instance->setAsMemberNID($nodeContent->getID(), $obfuscated, $index);
                }
            }
