...
//			'sylabeLoadModules'					=> self::APPLICATION_DEFAULT_LOAD_MODULES,
	...
	public static function getOption($name)
	{
		...
		switch ($name)
		{
			case 'sylabeDisplayOnlineHelp' :			if ( $t == 'false' )	$r = false;		else $r = self::APPLICATION_DEFAULT_DISPLAY_ONLINE_HELP;			break;
			case 'sylabeDisplayOnlineOptions' :			if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_DISPLAY_ONLINE_OPTIONS;			break;
			case 'sylabeDisplayMetrology' :				if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_DISPLAY_METROLOGY;				break;
//			case 'sylabeDisplayUnsecureURL' :			if ( $t == 'false' )	$r = false;		else $r = self::APPLICATION_DEFAULT_DISPLAY_UNSECURE_URL;			break;
			case 'sylabeDisplayUnverifyLargeContent' :	if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_DISPLAY_UNVERIFY_LARGE_CONTENT;	break;
			case 'sylabeDisplayNameSize' :				if ( $t != '' )			$r = (int)$t;	else $r = self::APPLICATION_DEFAULT_DISPLAY_NAME_SIZE;				break;
			case 'sylabeIOReadMaxDataPHP' :				if ( $t != '' )			$r = (int)$t;	else $r = self::APPLICATION_DEFAULT_IO_READ_MAX_DATA;				break;
			case 'sylabePermitUploadObject' :			if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_PERMIT_UPLOAD_OBJECT;			break;
			case 'sylabePermitUploadLinks' :			if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_PERMIT_UPLOAD_LINKS;			break;
			case 'sylabePermitPublicUploadObject' :		if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_PERMIT_PUBLIC_UPLOAD_OBJECT;	break;
			case 'sylabePermitPublicUploadLinks' :		if ( $t == 'true' )		$r = true;		else $r = self::APPLICATION_DEFAULT_PERMIT_PUBLIC_UPLOAD_LINKS;		break;
...
?>
		...
		<style type="text/css">
			...
			.sylabeMenuListContentActionModules { background:rgba(0,0,0,0.5); color:#ffffff; }
			.sylabeMenuListContentActionHooks { background:rgba(255,255,255,0.66); }
...
<?php
	...
	{
		if ( $this->_applicationInstance->getOption('sylabeDisplayMetrology') )
		{
?>
...
<?php
		switch ($status)
		{
			case 'notPresent':
				...
				break;
			case 'tooBig':
				if ( $this->_applicationInstance->getOption('sylabeDisplayUnverifyLargeContent') )
				{
...
<?php
	...
	private function _displayDivOnlineHelp($help)
	{
		...
		if ( $this->_applicationInstance->getOption('sylabeDisplayOnlineHelp') )
		{
...
class ModuleManage extends Modules
{
	...
	public function initialisation()
	{
		...
		$this->_unlocked		= $this->_nebuleInstance->getCurrentEntityUnlocked();
		$this->_initTable();
		$this->_hashModule		= $this->_nebuleInstance->getCrypto()->hash(nebule::REFERENCE_NEBULE_OBJET_INTERFACE_APP_MODULES);
	}
	...
	public function display()
	{
		...
		{
			case $this->MODULE_REGISTERED_VIEWS[0]:	$this->_displayModules();		break;
			case $this->MODULE_REGISTERED_VIEWS[1]:	$this->_displayModule();		break;
			case $this->MODULE_REGISTERED_VIEWS[2]:	$this->_displayCreateModule();	break;
			case $this->MODULE_REGISTERED_VIEWS[3]:	$this->_displayChangeCode();	break;
			default:								$this->_displayModules();		break;
		}
	}
	...
	public function displayInline()
	{
		...
		{
			case $this->MODULE_REGISTERED_VIEWS[0]:	$this->_display_InlineModules();	break;
			case $this->MODULE_REGISTERED_VIEWS[1]:	$this->_display_InlineModule();		break;
		}
	}
...
<?php
	...
	private function _extractActionAddModuleCode()
	{
			...
			)
		{
			...
			if ( $arg_id != ''
					&& ctype_xdigit($arg_id)
					&& $this->_nebuleInstance->getIO()->checkObjectPresent($arg_id)
					&& $this->_nebuleInstance->getIO()->checkLinkPresent($arg_id)
					&& $this->_nebuleInstance->getIO()->checkObjectPresent($arg_id)
					&& $this->_nebuleInstance->getIO()->checkLinkPresent($arg_id)
					&& strlen($arg_id) >= 6
					&& $arg_id != $arg_rid
	...
	private function _actionAddModule()
	{
			...
			)
		{
			...
			$instance = New Object($this->_nebuleInstance, $this->_actionAddModuleRID, '', false, false);
			...
			$target	= $this->_nebuleInstance->getCrypto()->hash( $this->_nebuleInstance->getCrypto()->hashAlgorithmName() );
			$meta	= $this->_nebuleInstance->getCrypto()->hash( nebule::REFERENCE_NEBULE_OBJET_HASH );
...
<?php
		...
		if ( $this->_unlocked
				...
				&& ( $this->_nebuleInstance->getCurrentEntity() == $this->_nebuleInstance->getCodeMaster()
						...
						&& $this->_nebuleInstance->getOption('permitInstanceEntityAsAuthority')
						|| $this->_nebuleInstance->getCurrentEntity() == $this->_nebuleInstance->getDefaultEntity()
						&& $this->_nebuleInstance->getOption('permitDefaultEntityAsAuthority')
...
<div class="sylabeModuleHelpText1stT">
	<p>
		<?php $this->_applicationInstance->getTraductionInstance()->echoTraduction('::sylabe:module:help:Bienvenue') ?><br />
	</p>
</div>
<?php
	}
...
class ModuleAdmin extends Modules
{
	...
	private function _displayAppOptions()
	{
		...
		if ( $this->_unlocked )
		{
			...
			foreach ($listOptions as $optionName)
			{
				...
				$optionValue			= $this->_applicationInstance->getOption($optionName);
				$optionID				= $this->_nebuleInstance->getCrypto()->hash($optionName);
	...
	private function _displayNebOptions()
	{
		...
		if ( $this->_unlocked )
		{
			...
			{
				// Extrait les propriétés de l'option.
				$optionValue			= $this->_nebuleInstance->getOption($optionName);
				$optionID				= $this->_nebuleInstance->getCrypto()->hash($optionName);
				$optionValueDisplay		= (string)$optionValue;
				$optionType				= $listOptionsType[$optionName];
